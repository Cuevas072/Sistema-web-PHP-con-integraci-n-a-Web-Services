...
if($session->checkSession() ){
    ...
    if(isset($_SESSION['fechaSesion'])){
        ...
        if($tiempoTranscurrido >= ($parametro['timeout'] * 60)){
            $session->endSession();
            header("Refresh:0");
            exit();
        }else{
    ...
    foreach($resultado_permisos as $fila){
        $varAccesos['idmenu'] = $fila['idmenu'];
        $varAccesos['nombre'] = $fila['nombre'];
        $varAccesos['ventana'] = $fila['ventana'];
        $varAccesos['framework'] = explode(",",$fila['framework']);        
    }
 
    #En caso de que el usuario no tenga permiso, verificamos en cual menu lo tiene

    if( count($varAccesos) == 0){
        ...
        foreach($resultadoAccpagina as $fila){
            $pagina = $fila['ventana'];      
            $flagAccPagina = true; 
            print_r($flagAccPagina);
        }

        if($flagAccPagina == false){
            $session->endSession();
            echo "Estimado, usted no tiene modulos asignados en el aplicativo, favor
            de conectar con el administrador del sistema";
            header("Refresh:10");
            exit();
        }else{
    ...
    if(count($varAccesos) > 0){
        $conexion->ejecutarConsulta("
            ...
            VALUES
            ('".Funciones::ObtenerIp()."','".Funciones::ObtenerNavegador($_SERVER['HTTP_USER_AGENT'])."','".$_SESSION['usuario']."','".$varAccesos['idmenu']."','".$varAccesos['nombre']."','".$varAccesos['ventana']."',NOW())
